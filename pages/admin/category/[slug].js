...
const Update = ({oldCategory, token}) => {
	...
	const handleSubmit = async (e) => {
		...
		try{
			...
			setState({...state, imagePreview: response.data.image.url, success:`${response.data.name} is updated`});
			setContent(response.data.content);
		} catch(error){
			console.log('Category update error',error);
			setState({...state, buttonText:'Update Image', error: error.response.data.error});
		}
	};
